...
import com.tennis.broadcaster.ATP_CHALLENGERS;
...
public class IndexController 
{
	...
	public static ATP_CHALLENGERS this_ATP_Challengers;
...
//		        System.out.println("Name : "+name);
			...
			throws IOException, ParseException, JAXBException, InterruptedException  
	{
		if(current_date == null || current_date.isEmpty()) {
			...
			case TennisUtil.ATP_2022:
				session_selected_scenes.add(new Scene(TennisUtil.ATP_2022_SCOREBUG,TennisUtil.FRONT_LAYER)); // Front layer
				session_selected_scenes.add(new Scene("",TennisUtil.MIDDLE_LAYER));
				...
				break;
			case TennisUtil.ATP_CHALLENGERS:
				session_selected_scenes.add(new Scene(TennisUtil.ATP_CHALLENGERS_SCOREBUG,TennisUtil.FRONT_LAYER)); // Front layer
				session_selected_scenes.add(new Scene("",TennisUtil.MIDDLE_LAYER));
				session_selected_scenes.get(0).scene_load(print_writer, session_selected_broadcaster);
				print_writer.println("-1 RENDERER*STAGE SHOW 0.0\0");
				this_ATP_Challengers = new ATP_CHALLENGERS();
				this_ATP_Challengers.scorebug = new ScoreBug();
				break;
			case TennisUtil.TPL_2023:
				session_selected_scenes.add(new Scene("/Default/ScoreBug_New02","FRONT_LAYER")); // Front layer
			...
			throws IllegalAccessException, InvocationTargetException, JAXBException, IOException
	{
		if (request.getRequestURI().contains("upload_match_setup_data") 
			...
			System.out.println("session match file name : "+session_match.getMatchFileName());
			MatchFile = session_match.getMatchFileName();
	...
	{	
		...
		case "FF_PLAYERPROFILE_GRAPHICS-OPTIONS": case "LT_PLAYERPROFILE_GRAPHICS-OPTIONS": case "MATCH_FF_PLAYERPROFILE_GRAPHICS-OPTIONS": case "MATCH_LT_PLAYERPROFILE_GRAPHICS-OPTIONS":
		case "TIE_RESULT_GRAPHICS-OPTIONS": case "ORDER_OF_MATCH_GRAPHICS-OPTIONS": case "LT-TEAM_GRAPHICS-OPTIONS": case "LT_SINGLE-MATCHPROMO_GRAPHICS-OPTIONS": 
		case "LT_DOUBLE-MATCHPROMO_GRAPHICS-OPTIONS":
			...
			case TennisUtil.ATP_CHALLENGERS:
				return (String) this_ATP_Challengers.ProcessGraphicOption(whatToProcess, session_match, tennisService, print_writer, session_selected_scenes, valueToProcess);
			case TennisUtil.TPL_2023:
		...
		case "APIDATA_GRAPHICS-OPTIONS":
			...
			return objectMapper.writeValueAsString(TennisFunctions.getMatchStatsApi(link));
		...
		case TennisUtil.READ_CLOCK:
			...
			case TennisUtil.ATP_2022:
				...
				break;
			case TennisUtil.ATP_CHALLENGERS:
				this_ATP_Challengers.updateScoreBug(session_selected_scenes, session_match, print_writer);
				break;
			}
			return objectMapper.writeValueAsString(session_match);
		...
		case TennisUtil.LOAD_MATCH:
			...
			case TennisUtil.LOAD_MATCH:
				switch (session_selected_broadcaster) {
				case TennisUtil.ATP_2022: case TennisUtil.ATP_CHALLENGERS:
		...
		case "READ-MATCH-AND-POPULATE":
			if(session_match != null && !valueToProcess.equalsIgnoreCase(new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(
					new File(TennisUtil.TENNIS_DIRECTORY + TennisUtil.MATCHES_DIRECTORY + session_match.getMatchFileName()).lastModified())))
			{
				if(session_match.getMatchFileName()==null) {
					session_match.setMatchFileName(MatchFile);
				}
				System.out.println("session_match.getMatchFileName()"+session_match.getMatchFileName());
				session_match = TennisFunctions.populateMatchVariables(tennisService, new ObjectMapper().readValue(
						new File(TennisUtil.TENNIS_DIRECTORY + TennisUtil.MATCHES_DIRECTORY + session_match.getMatchFileName()), Match.class));					
				
				session_match.setMatchFileTimeStamp(new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(
						new File(TennisUtil.TENNIS_DIRECTORY + TennisUtil.MATCHES_DIRECTORY + session_match.getMatchFileName()).lastModified()));
				
				session_event = new ObjectMapper().readValue(new File(TennisUtil.TENNIS_DIRECTORY + TennisUtil.EVENT_DIRECTORY +
		...
		default:
			...
			case TennisUtil.ATP_2022:
				...
				break;
			case TennisUtil.ATP_CHALLENGERS:
				this_ATP_Challengers.ProcessGraphicOption(whatToProcess, session_match, tennisService, print_writer, session_selected_scenes, valueToProcess);
				break;
			case TennisUtil.TPL_2023:
				...
				return objectMapper.writeValueAsString(this_TPL_2023);
			}
			return objectMapper.writeValueAsString(session_match);
		}
	}
